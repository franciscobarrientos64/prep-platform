...
- **client-name.js** — pone el nombre del cliente activo en el header de cada módulo + selector de sede (marca con 2+ locales) y de restaurante. El selector de restaurante lo ve quien tiene 2+ marcas en `prep_usuario_marcas` y, con **todas** las marcas, el `superadmin`: en `os.prep.rest` cambia de cliente con `?marca=&local=` sin salir de la pantalla; dentro de un subdominio de cliente salta al subdominio del otro.
...
- **presentacion.html** (`/presentacion`, alias `/comite`) — **Comité de Experiencia**: deck ejecutivo multi-marca de mystery shopper, gated a `rol_sistema='superadmin'` (no se enlaza desde el hub de clientes). **Se entra por dos lados:** la sección **Voz del Cliente** de `/portal` (desplegable con las marcas que tienen evaluaciones → deck consolidado o de una marca, + acceso al módulo de esa marca) y el botón **"Comité de Experiencia"** del encabezado de `voz-cliente.html`, que lleva al deck de la marca activa (`initSuper()` lo muestra solo al superadmin y avisa si el cliente que estás mirando no tiene visitas cargadas). 13 láminas navegables (← →, chips de índice, barra de progreso) + export PDF del deck completo (`printAll()` dibuja todos los charts antes de imprimir). Se alimenta sola de `mys_visitas/mys_respuestas/mys_detalle/mys_acciones/mys_criterios` — cualquier marca con data de mystery entra automáticamente; el selector de periodo lista todos los meses disponibles. Láminas: portada, panorama (KPIs + barras jun/jul + delta), comparativo por marca (score + brecha interna + evolución + radar por mundo), evolución (serie completa + recorrido acumulado), ranking (mundo más fuerte/débil + rango entre visitas), totales por mundo (heatmap mundo × operación con peso), **dónde se pierden los puntos** (peso × brecha, barras apiladas), **consistencia** (dispersión peor/mejor visita), **oportunidades por mundo** (matriz ítem × operación ordenada por incidencia, con etiquetas transversal/puntual), **transversal vs puntual**, plan de acción (top 10 por operación desde `mys_acciones`), **proyección** (techo alcanzable al cerrar las 10 prioridades; base = auditoría, no el ponderado de visitas) y cierre con acuerdos. Mapa `M2C` traduce mundo (lenguaje del cliente) → criterio ponderado.
  **Deck por marca:** `?marca=m7` / `?marca=m8` (chips en el header) generan presentaciones independientes; sin el parámetro consolida todas. Con una sola operación los KPIs pasan a mundo fuerte/débil + rango entre visitas, el ranking se vuelve "ficha de la operación" y se oculta la lámina transversal.
  **⚠️ Ítems de conducta (redacción negativa):** "No solicita propina", "No sintió maltrato/discriminación", "No sientes que te apuran", "No presenta olores molestos", "Sientes que el mesero fue respetuoso" y los de alergias tienen la escala invertida — **4 = el equipo actuó bien, 0 = ocurrió la falta**. Un 0 ahí acusa al personal de una inconducta, no es una oportunidad de mejora. La constante `CRITICOS` los aísla en la lámina **Conducta y cumplimiento crítico** y los etiqueta si aparecen en otras láminas. **Nunca generar ni degradar estos ítems al simular data.**
...
### Multi-cliente (al 2026-06-15) — Symposium y La Cuadra de Salvador
- **Clientes nuevos:** Symposium (`m7`/`l12`, subdominio `symposium.prep.rest`) y **La Cuadra de Salvador** (`m8`, subdominio `lcds.prep.rest`, **4 locales**: San Isidro `l13`, Barranco `l14`, Chacarilla `l15`, La Molina `l16`). `tenant.js` mapea ambos subdominios + setea `window.PREP_BYSUB`.
- **Paquetes por cliente** (`prep_marca_modulos`, gating en el hub `enParquete`): Symposium = Mystery + El Libro + RRHH (Directorio **off**); La Cuadra = solo Mystery. Módulos fuera del plan se ven bloqueados con teaser de features. Keys de módulo separados `mystery`/`prediccion`.
- **Voz del Cliente** (`voz-cliente.html`, rutas `/voz-cliente`, `/voz`, alias `/mystery`): contenedor con **3 pestañas superiores** (`setMode`): Mystery Shopper · Encuestas · Reseñas. Key de módulo sigue siendo `mystery` (gating en hub vía `MODKEY['Voz del Cliente']='mystery'`). `mystery.html` fue renombrado a este archivo.
  - **Pestaña Mystery Shopper** (ex `mystery.html`): Chart.js (barra/donut/línea/radar + datalabels), filtro por mes, vista "Todas las sedes" (consolidado), Resumen/Tendencias/Radar/Detalle/Auditoría/Plan + Export PDF ejecutivo. Tablas `mys_plantillas/mys_criterios/mys_visitas/mys_respuestas/mys_detalle/mys_acciones`. Pesos ocultos. Datos reales: La Cuadra (112 visitas + auditoría ítem-por-ítem mayo 2026: 234 ítems × 4 sedes = 936 filas en `mys_detalle`, periodo `2026-05`), Symposium (17 visitas + 229 ítems). **Periodo `2026-07` cargado (ago 2026):** 20 visitas + 220 respuestas + 1.165 ítems de auditoría + 50 acciones (10 por operación). Deltas vs junio: Symposium 0.0 (meseta), San Isidro +0.5, Barranco +1.2, La Molina +0.3, Chacarilla −0.6. **Periodo `2026-08` simulado (sep 2026):** deltas vs julio Symposium +3.9, San Isidro +1.3, Barranco +0.8, Chacarilla −3.2, La Molina −2.8 (visita del 8-ago con arañita en la mesa, mesa sin setear y hielo pedido 3 veces que no llegó). En agosto la sección **Vino** de La Cuadra usa "no aplica" (NULL): botella siempre N/A; si se acercó el sommelier, el ítem del mesero es N/A y viceversa; si nadie ofreció vino, maridaje/servicio N/A; "lo sirve correctamente" solo si tomaron copa (mayo–julio siguen sin esa lógica). **Gráficos (sep 2026):** Resumen muestra "Evolución del puntaje total" para todos los clientes; ese gráfico y "Evolución por sede" usan eje Y acotado a los puntajes (`zoomY`); las tablas de Detalle cierran con la fila "Puntaje total" (`heat(...,{total})`). El detalle de La Cuadra usa la terminología del cliente en `mundo` (Reserva, Estacionamiento, Bienvenida y despedida, Infraestructura, Mesa, Alimentos & Bebidas, Mesero, Vino, Pago, Administración, Lealtad).
  - **Pestaña Encuestas** (VC-05..09): constructor de encuestas (tipos nps/csat/escala5/opciones/abierta), lista con NPS/CSAT por encuesta, resultados (donut NPS, satisfacción por pregunta, opciones, comentarios), QR/link + compartir WhatsApp, activar/pausar. Tablas `vc_encuestas/vc_preguntas/vc_respuestas/vc_resp_items` (RLS tenant + anon: lee solo encuestas activas, inserta respuestas, NO lee respuestas). Página pública de respuesta: **`encuesta.html`** (ruta `/encuesta?id=`, sin auth-guard, soporta `?mesa=&mozo=` + campos de contacto opcionales nombre/teléfono). **Resultados en tiempo real** (Realtime sub a `vc_respuestas`) y **vista de Alertas**: respuestas con NPS≤6 o CSAT≤2 se listan con mesa/mozo/contacto + botón Contactar (WhatsApp) + marcar atendida. Columnas extra en `vc_respuestas`: `contacto_nombre/telefono`, `atendida/atendida_at/atendida_por`; `vc_encuestas.en_ticket` marca la encuesta que el **POS imprime como QR en la pre-cuenta** (`pos-v2.html` imprimirPreCuenta, precarga la imagen). Encuesta tipo activa de Casa Italia: **"Experiencia Casa Italia"** (NPS + 4 CSAT de operación + diagnóstico + abierta). Manual de instalación en super admin: **`instalacion.html`** (ruta `/instalacion`, gated `prep_is_super`, renderiza `docs/INSTALACION.md`; tile en `/portal`). Docs en `docs/INSTALACION.md` + `docs/AUDITORIA.md`. Consola super admin **`onboarding.html`** (ruta `/onboarding`, tile en `/portal`): por cliente, checklist de información a solicitar (equipos/red/usuarios/integraciones, guardado en tabla `prep_onboarding` con RLS tenant) + estado en vivo de `config_local` (qué falta) con link a `/ajustes`.
  - **Pestaña Reseñas** (VC-10..16, tabla `eng_resenas`): monitor Google/TripAdvisor (rating real, sin responder, por fuente/estado), filtros, **respuesta con IA** (borrador template por rating/contexto — la publicación es manual; falta API key Claude + Google My Business/TripAdvisor API para integración real). Casa Italia tiene 8 reseñas reales.
- **El Libro ampliado:** Equipo, Exámenes (panel de notas), Calendario (eventos + asistencia, `lb_eventos.asistentes`), The Talk (`lb_thetalk`, alta/edición), Políticas, **Briefing diario** (`lb_briefings`), **Eventualidades** (`lb_eventualidades`), checklists Apertura/Cierre. Usa **invitado** (no comensal).
- **Cursos propios del cliente (ago 2026):** tabla `lb_cursos(marca_id,local_id,slug,titulo,descripcion,emoji,url,grupo,orden,activo,publicado_at,publicado_por)` con RLS tenant (`prep_can_local`). El Libro → **Cursos** muestra primero el bloque **"Cursos de la casa"** (los del cliente) y debajo el catálogo global `CATALOGO` de la Academia. Un curso con `activo=false` es **borrador**: solo lo ve la gerencia (`rol_sistema` superadmin/admin_marca, detectado en `loadRol()` → `state.esAdmin`), con el botón **Habilitar / Ocultar** (`toggleCursoCasa`). El equipo no lo ve ni puede abrirlo: la propia página del curso revalida `lb_cursos.activo` y muestra "Todavía no" si sigue en borrador (el superadmin siempre entra, para revisarlo antes de publicar).
- **Exámenes publicables:** `loadExamenes()` ya NO filtra `activo=true`; el filtro es de render. Un examen con `activo=false` sale con la etiqueta **Borrador** y solo lo ve la gerencia, con el botón **Publicar / Ocultar** (`toggleExamen`). Así se prepara el examen antes de abrirlo al equipo.
- **Symposium (m7) — carta de bebidas (ago 2026):** curso `/cursos/symposium-bebidas.html` (16 tragos de la carta: 6 sours, 2 bitters, 2 highballs, 2 spritz, 4 mocktails; 7 módulos + one-pager imprimible; paleta azul noche + oro del sello del cliente). Fuente: PDF `CARTA BEBIDAS DE SYMPOSIUM_17.3X29`.
  - **Nivel 1 (32 preguntas, en borrador) — DOS PUNTAJES SEPARADOS.** *Parte 1 · Conocimiento*: 16 de opción múltiple, `¿Qué lleva el X?`, y las 4 opciones son **combinaciones completas de ingredientes**; los distractores son recetas reales de la misma carta o la correcta con UN insumo cambiado, así no se adivina por descarte (pasar 12/16 adivinando = 0,004%). *Parte 2 · Venta sugestiva*: 16 preguntas **abiertas**, una línea de venta por cóctel; una línea vale si tiene **más de 4 palabras** (`MIN_PAL=5`), con contador en vivo bajo cada campo. Al entregar salen los dos puntajes por separado (cada uno aprueba con 70%); las líneas no se autocalifican, las lee el bar. **Nivel 2 · técnica y servicio** (20 preguntas) queda para después.
  - ✅ **Fidelidad al PDF**: las 16 respuestas correctas usan solo palabras que aparecen en la carta (verificado por script contra el texto del PDF). La carta trae erratas — *cramberry*, *Shitake*, *Shrup*, *kalhua*, *Jeréz*, *hierba buena*, *Infusion* — que en el material se escriben bien (cranberry, shiitake, sirope, Kahlúa, Jerez, hierbabuena, infusión). El Amargo Mandarino **no declara amaro entre sus insumos** (la carta solo dice Pisco, naranja amarga y hierbas): no inventarlo.

### Motor de exámenes de El Libro (ago 2026)
- **Dos tipos de pregunta** en `rrhh_examenes.preguntas`: opción única `{q, opciones, correcta}` y abierta `{q, tipo:'abierta', placeholder}`. Cualquier pregunta puede abrir una sección con `{seccion, ayuda}` (se pinta como cabecera dentro del formulario).
- `tomarExamen()` ya **no repinta el formulario en cada clic** (rompía los textarea): cosecha los textos con `harvest()` y solo alterna clases en los botones. Contador de palabras en vivo por respuesta abierta.
- **Calificación:** *Conocimiento* = aciertos / preguntas de opción única. *Venta sugestiva* = líneas con más de 4 palabras / preguntas abiertas. Son puntajes independientes, cada uno con corte en 70%. No hay nota combinada.
- **`rrhh_examen_resultados`** guarda además `respuestas` (jsonb, todo lo que marcó y escribió), `escritas`, `escritas_total`, `pct` (= % de conocimiento) y `pct_ventas` (= % de venta sugestiva).
- **Revisión (`verEntrega`)**: la gerencia toca un resultado y lee las líneas que escribió la persona + sus errores de conocimiento. Los operativos no ven ese detalle.
- Sigue sin existir la **multi-selección** (marcar N insumos de una lista larga); si hiciera falta, es un tipo `multi` con `correctas:[]` y regla de crédito parcial.
- **Reporte de cierre por email:** Edge Function `reporte-diario` (verify_jwt=false) arma el reporte del día (eventualidades + turno + horas de briefing y checklists) y lo envía a `config_local.reporte_emails` vía Resend. Cron `pg_cron` `reporte-diario-cierre` a las 04:30 UTC (23:30 Lima); botón "Enviar reporte de hoy" en El Libro. **Pendiente dueño:** `RESEND_API_KEY` + dominio verificado para que envíe de verdad.
- **Branding por marca:** `client-name.js` fija nombre + título + **logo del restaurante** junto al nombre (`/logos/m7.png` Symposium, `/logos/m8.png` La Cuadra). Texto "Casa Italia" removido de headers/títulos de todos los módulos.
- ⚠️ **Crear cuentas a mano en `auth.users`: los tokens van en `''`, NUNCA en NULL.** Si se omiten `confirmation_token`, `recovery_token`, `email_change`, `email_change_token_new`, `email_change_token_current`, `phone_change`, `phone_change_token` y `reauthentication_token`, quedan en NULL y el login falla con **"Database error querying schema"** — GoTrue los lee como texto no nulo y el scan revienta. No es clave incorrecta ni falta de identidad: es este NULL. Pasó el 21-ago-2026 con las cuentas de Stevens Espinoza y Anthony Herrera. Diagnóstico y arreglo: `update auth.users set confirmation_token=coalesce(confirmation_token,''), … where <alguna> is null`. Súmalos siempre al INSERT de [[reference-prep-crear-accesos]].
- **"Ver como" — vista de rol del super admin (ago 2026):** script compartido **`role-view.js`** (va en el `<head>` de los 21 módulos, junto a `client-name.js`, y también en las consolas super admin). Pone un botón flotante abajo a la izquierda **"Ver como"** que solo ve el `superadmin`; al elegir un rol la app entera se comporta como ese rol y aparece una **barra amarilla fija** ("Estás viendo la app como X" + Cambiar rol + Salir de la vista).
  - El rol elegido vive en `sessionStorage.prep_vista` (se va al cerrar la pestaña) y trae `{rol_sistema, rol_id, nombre, permisos}` **completo**, así `window.PREP_VISTA` se resuelve **síncrono** y los módulos pueden leerlo mientras pintan.
  - API para los módulos: `window.PREP_VISTA`, `prepNivel(modulo)` → ninguno|ver|operar|aprobar|admin, `prepEsAdminVista()`, `prepFormatos()`. Todas devuelven `null` cuando no hay vista activa (= usa el rol real).
  - Bloqueo por pantalla: `RUTA_MOD` mapea ruta → módulo; si el rol simulado no llega (o el módulo no está en `prep_marca_modulos` del cliente) se tapa la página con un aviso. Las consolas de Prep (`SOLO_SUPER`: /portal /prep /admin /prep-negocio /presentacion …) se bloquean siempre bajo una vista. La barra amarilla queda por encima del bloqueo, así nunca te quedas encerrado.
  - Enganches: el hub (`nivelDeMod`, `moduloActivo`, chip de cuenta) y El Libro (`state.esAdmin`, pestañas por formato).
  - **No es escalada de privilegios**: solo restringe lo que se ve. La data sigue gobernada por RLS con la sesión real, así que forzar la llave de sessionStorage no abre nada.
- **Roles y formatos por rol (ago 2026):** `prep_roles.permisos` acepta ahora, además de `modulo: nivel`, una llave reservada **`formatos: [...]`** con las pestañas de El Libro que ve ese rol (`tablon, equipo, procedimientos, recetario, plantillas, cursos, examenes, briefing, checklists, eventualidades, calendario, thetalk, politicas, bitacora`). Sin la llave, el rol ve todas. Se edita con chips en **`/admin` → Roles & permisos** (constante `FORMATOS` en admin.html); El Libro filtra sus pestañas con `state.formatos` (del rol real o de la vista).
  - **Roles de Symposium (m7):** **Jefe de Servicio** (`libro:operar`, `mystery:ver`, `rrhh:ver`; 11 formatos — todos menos recetario, plantillas y políticas) y **Equipo de Sala** (`libro:ver`; 7 formatos de lectura). Casa Italia (m6) tiene Cajero/Chef/Mozo/Almacén/Contador.
  - ⚠️ **Los 8 operativos de Symposium siguen con `rol_id=null`** → con el rol real no ven ningún módulo en el hub. Hay que asignarles rol cuando llegue la lista nueva de usuarios.
- **Accesos:** SSO entre subdominios `*.prep.rest` (cookie de dominio en `auth-guard.js`). Desplegable de staff en el login **solo en URLs de cliente** (`prep_login_roster`, gated por `PREP_BYSUB`). Usuarios operativos de Symposium (8) con cambio de clave obligatorio (`prep_usuarios.must_change_pw`). Grupo dueño (4) con acceso a m7+m8 (`prep_usuario_marcas` + helpers `prep_can_marca/local` extendidos).
- **Catálogo de features:** `features.html` (ruta `/features`) — branded, filtrable por fase, buscable.
- **Brand book:** `brand.html` (ruta `/brand`). Logo del wordmark = fuente **Stellost Slanted Regular** (archivo `/logo.png`).
