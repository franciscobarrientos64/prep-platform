...
- **Mystery Shopper** (`mystery.html`, ruta `/mystery`): módulo nuevo. Chart.js (barra/donut/línea/radar + datalabels), filtro por mes, vista "Todas las sedes" (consolidado), Resumen/Tendencias/Radar/Detalle (4 visitas del mes juntas)/Auditoría (ítem×color 0-4, sin nota)/Plan. Tablas `mys_plantillas/mys_criterios/mys_visitas/mys_respuestas/mys_detalle/mys_acciones`. **Pesos ocultos** (ponderan internamente; nunca se muestran). Datos reales cargados: La Cuadra (112 visitas, 7 meses) y Symposium (17 visitas + detalle ítem de mayo, 229 ítems).
