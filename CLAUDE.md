...
- **Symposium (m7) — carta de bebidas (ago 2026):** curso `/cursos/symposium-bebidas.html` (16 tragos de la carta: 6 sours, 2 bitters, 2 highballs, 2 spritz, 4 mocktails; 7 módulos + one-pager imprimible; paleta azul noche + oro del sello del cliente). Fuente: PDF `CARTA BEBIDAS DE SYMPOSIUM_17.3X29`.
  - **Exámenes en dos niveles, ambos en borrador.** **Nivel 1 · "¿qué lleva cada trago?"** (16 preguntas, una por cóctel, solo ingredientes): el enunciado es `¿Qué lleva el X?` y las 4 opciones son **combinaciones completas de ingredientes**, no ingredientes sueltos. Los distractores se arman con recetas reales de la misma carta o cambiándole UN insumo a la correcta, así no se adivina por descarte. Aprobando con 12/16, la probabilidad de pasar adivinando es 0,004%. **Nivel 2 · "técnica y servicio"** (20 preguntas: precios, batido vs refrescado, alérgenos, situaciones de mesa) queda guardado para después del Nivel 1.
  - ⚠️ **El motor de exámenes de El Libro solo grada opción única** (`{q, opciones, correcta}`, se compara `resp[i]===p.correcta`). Multi-selección (marcar N insumos de una lista larga) o respuesta escrita **no existen todavía**: harían falta un tipo de pregunta nuevo en `tomarExamen`/`crearExamen` y, en el caso escrito, una pantalla de corrección manual.
