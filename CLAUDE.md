...
- **PENDIENTE seguridad**: barrer políticas `always_true` en tablas Prep (ca_pedidos, ca_pedido_items, ca_cortesias, ca_gift_cards, config_local, inv_*, dl_pedidos, li_estaciones, ms_card_holds/visitas/cliente_fechas, prep_marca_features/modulos, prep_roles/usuarios, rrhh_*) → reemplazar por el patrón ya probado en `ca_audit` (`USING/CHECK prep_can_local(local_id)` o `prep_can_marca(marca_id)` para `authenticated`). **Hacerlo en branch de Supabase y probar antes de mergear** (un error tumba el POS en vivo). Bucket `recetas-files` queda público a propósito (fotos de la carta pública). **Rotar service_role key + token GitHub** y activar HIBP/MFA/2FA → acción del dueño en el dashboard.
