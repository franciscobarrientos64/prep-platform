...
- **Pasarela de pago (#7, scaffold):** `config_local.{pasarela_tipo,pasarela_public_key,pasarela_activa}` editable en `/ajustes`; tabla `ca_pagos_online`; Edge Function **`cobro-tarjeta`** (verify_jwt) con Culqi listo (lee `CULQI_SECRET_KEY` del entorno). **Pendiente activación:** elegir pasarela + cargar llaves sandbox + enlazar el botón en el POS (pos-v2).
